...
let baseCat = [];
if (fs.existsSync('./scripts/master-seed-catalogue.json')) {
  baseCat = JSON.parse(fs.readFileSync('./scripts/master-seed-catalogue.json', 'utf-8'));
}
if (fs.existsSync('./src/data/anivault-catalogue.json')) {
  ...
  if (current.length > baseCat.length) {
    baseCat = current;
  }
}
...
// Save verified catalogue to src/data, server/data, and master seed
fs.writeFileSync('./scripts/master-seed-catalogue.json', JSON.stringify(finalCatalogue, null, 2));
